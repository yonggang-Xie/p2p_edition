...
class RealImageEditor:
    ...
    def _run_editing(self, prompts: List[str], controller, latent: torch.Tensor,
        ...
        te_dtype   = next(te.parameters()).dtype

        # Move *all* tokenizer outputs that exist
        tok = text_input  # whatever you named it earlier
        move_keys = []
        if hasattr(tok, "input_ids"):       move_keys.append("input_ids")
        if hasattr(tok, "attention_mask"):  move_keys.append("attention_mask")
        if hasattr(tok, "position_ids"):    move_keys.append("position_ids")

        for k in move_keys:
            setattr(tok, k, getattr(tok, k).to(device=te_device))

        # Encode on text-encoder device
        with torch.no_grad():
            text_embeddings = te(
                input_ids=getattr(tok, "input_ids", None),
                attention_mask=getattr(tok, "attention_mask", None),
                position_ids=getattr(tok, "position_ids", None),
            )[0]  # (batch, seq, dim)

        # Now move embeddings to the diffusion device (self.device) & dtype expected downstream
